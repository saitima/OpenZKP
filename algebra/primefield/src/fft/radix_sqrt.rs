use super::{bit_reverse::permute_index, fft_vec_recursive_with_unroll, get_twiddles, recursive::fft_vec_recursive, transpose::transpose_square_stretch};
...
pub fn radix_sqrt_with_unroll<Field, const MAX_UNROLL: usize>(values: &mut [Field], root: &Field)
where
    Field: FieldLike + Send + Sync,
    for<'a> &'a Field: RefFieldLike<Field>,
{
    ...
    values
        .par_chunks_mut(outer)
        .for_each(|row| fft_vec_recursive_with_unroll::<_, MAX_UNROLL>(row, &twiddles, 0, stretch, stretch));
    trace!("END FFT Batch Recursive");
    ...
    values
        ...
        .for_each(|(i, row)| {
            ...
            fft_vec_recursive_with_unroll::<_, MAX_UNROLL>(row, &twiddles, 0, 1, 1)
        });
    trace!("END FFT Batch Recursive twiddles");
    trace!("END FFT Radix SQRT");
}
